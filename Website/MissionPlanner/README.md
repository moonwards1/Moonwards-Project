...
| `lunar-departure.js`     | `flyLunarDeparture`, `releaseVelocity`, `releaseSpeedFor`          | The departure flight from the Moon, flown FORWARD to Earth's SOI edge (Earth + Moon + Sun, `Shared/geo-leg.js`). Given a release epoch and an impulse in the Moon's own geocentric frame, it starts the ship one Moon-SOI radius along the way it was thrown, carrying the Moon's velocity plus that impulse, and integrates until it crosses Earth's SOI; where, how fast and WHEN are all outputs. A Moon origin is the one origin whose departure leaves a different body's sphere of influence than the one it started from. Impacts and captures are reported with the flight attached, so a caller can still draw what happened. Pure. |
...
- **`ephemeris-view.js`** — the Ephemeris tab: a scratchpad for authoring a
  ...
  impulse in the Moon's own geocentric frame, its date is the RELEASE, and
