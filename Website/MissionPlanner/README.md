...
| `departure-estimate.js`  | (see header — estimates the departure leg's duration)              | How long the departure leg lasts, estimated from the frozen plan alone (before any departure tech is chosen): the SOI radius over the TRUE hyperbolic excess behind the hand-off's edge speed. Feeds the read-only release anchor `freeze.js` bakes into the plan, the Ephemeris tab's Moon-phase widget, and the Departure slider's default span. Nothing here shapes a LUNAR departure — that one is solved from the card by `lunar-departure.js`; the Moon branch is a seed for a plan that arrived without one, covering lunar distance out to Earth's SOI rather than Earth's surface out. |
| `lunar-departure.js`     |  `flyLunarDeparture`, `cardVInf`, `vInfFromState`, `solveShipVelocity`, `hyperbolicCoastTime`, `releaseSpeedFor` | What a departure from the Moon is worth at Earth's SOI. The card holds the v∞ the SHIP's own actions deliver there — on EARTH's heliocentric axes, the same frame and meaning every other origin's card carries — and nothing the Moon contributes is in it. The Moon's own motion rides along for free but is handed over deep inside Earth's well, so what reaches Earth's SOI is a RESIDUAL, roughly 0.9–1.9× the Moon's speed and swinging right round with its phase. The two shares cannot be propagated separately (squares do not split; the Moon's 1,022 m/s alone does not escape), so the chain runs through the ship's velocity at the Moon: invert the card to it, add the Moon's velocity there, take the sum out through Earth's well exactly. Supports departures heading AWAY from Earth; everything else is refused by name. Pure. |
