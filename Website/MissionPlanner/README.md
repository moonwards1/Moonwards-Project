...
| `departure-estimate.js`  | (see header — estimates the departure leg's duration)              | How long the departure leg lasts, estimated from the frozen plan alone (before any departure tech is chosen): the SOI radius over the TRUE hyperbolic excess behind the hand-off's edge speed (`asymptoticVInf`; `edgeVInf` converts back for Target mode). Feeds the read-only release anchor `freeze.js` bakes into the plan, the Ephemeris tab's Moon-phase widget, and the Departure slider's default span. Nothing here shapes a LUNAR departure — that one is solved from the card by `lunar-departure.js`; the Moon branch is a seed for a plan that arrived without one, covering lunar distance out to Earth's SOI rather than Earth's surface out. |
| `lunar-departure.js`     |  `flyLunarDeparture`, `cardVInf`, `vInfFromState`, `solveShipVelocity`, `hyperbolicCoastTime`, `releaseSpeedFor`, `flyEarthPassDeparture` | What a departure from the Moon is worth at Earth's SOI. The card holds what the SHIP's own actions deliver there, as a speed AT the SOI edge (converted to the hyperbolic excess before the inversion, since only an escaping share can be inverted) — on EARTH's heliocentric axes, the same frame and meaning every other origin's card carries — and nothing the Moon contributes is in it. The Moon's own motion rides along for free but is handed over deep inside Earth's well, so what reaches Earth's SOI is a RESIDUAL, roughly 0.9–1.9× the Moon's speed and swinging right round with its phase. The two shares cannot be propagated separately (squares do not split; the Moon's 1,022 m/s alone does not escape), so the chain runs through the ship's velocity at the Moon: invert the card to it, add the Moon's velocity there, take the sum out through Earth's well exactly. Supports departures heading AWAY from Earth, and a share that escapes Earth on its own (below 929 m/s it cannot be stated separately from the Moon's); everything else is refused by name, drawn not at all, and routed through the `flyEarthPassDeparture` placeholder where the dive-past-Earth case will be solved. Pure. |
...
- **`mission-view.js`** exports `createMissionView({ world, registry,
  renderer, container, template, missionId, defaultMain, plan })` — everything
...
- **`ephemeris-view.js`** — the Ephemeris tab: a scratchpad for authoring a
  ...
  (authoring) or adopted from a pasted mission's own departure chain. How much
  impulse that hand-off costs, and when the launch must happen, are read
  ...
  A **Moon origin states only the ship's half of it.** Its card is the v∞ the
  SHIP's own actions deliver at Earth's SOI — on the same Earth-heliocentric
  axes every other origin's card uses, so "prograde" means one thing across the
  tab — and the Moon's own motion is deliberately absent from it, because the
  ship gets that for free. `core/lunar-departure.js` works out what that free
  motion is still worth once Earth's well has been climbed, and the drawn arc
  uses card plus that residual. So the same card a fortnight later is a
  different trajectory, while the card keeps reading what the ship must supply.
  Departures heading away from Earth are supported; the rest are refused by
  name and drawn not at all (`Notes/decisions.md`, 2026-09-02).
  ...
  mission tab (`onOpenPastedMission`), because a committed plan with a
  technology stack behind it is a mission and not a sketch. A one-set link
...
`core/world.js`'s `WORLD_VERSION` is 5; `deserializeWorld` refuses (politely,
