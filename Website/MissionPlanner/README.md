...
| `departure-estimate.js`  | (see header — estimates the departure leg's duration)              | How long the departure leg lasts, estimated from the frozen plan alone (before any departure tech is chosen): the SOI radius over the TRUE hyperbolic excess behind the hand-off's edge speed. Feeds the read-only release anchor `freeze.js` bakes into the plan, the Ephemeris tab's Moon-phase widget, and the Departure slider's default span. Nothing here shapes a LUNAR departure — that one is flown forward from a release the planner set (`lunar-departure.js`); the Moon branch is a seed for a plan that arrived without one, covering lunar distance out to Earth's SOI rather than Earth's surface out. |
| `lunar-departure.js`     |  `flyLunarDeparture`, `releaseImpulse`, `releaseVelocity`, `releaseSpeedFor` | The departure flight from the Moon, flown FORWARD to Earth's SOI edge (Earth + Moon + Sun, `Shared/geo-leg.js`). Given a release epoch and an impulse on EARTH's heliocentric axes — the same frame every other origin's departure card uses, so `prograde` means one thing across the tab — it starts the ship one Moon-SOI radius along the way it was thrown, carrying the Moon's velocity plus that impulse, and integrates until it crosses Earth's SOI; where, how fast and WHEN are all outputs. A Moon origin is the one origin whose departure leaves a different body's sphere of influence than the one it started from. Impacts and captures are reported with the flight attached, so a caller can still draw what happened. Pure. |
...
- **`ephemeris-view.js`** — the Ephemeris tab: a scratchpad for authoring a
  ...
  A **Moon origin is the exception, and runs the other way.** Its card is an
  impulse thrown from the Moon — on the same Earth-heliocentric axes every
  other origin's card uses, so "prograde" means one thing across the tab — its
  date is the RELEASE, and
  `core/lunar-departure.js` flies Earth + Moon + Sun forward from the Moon's
  SOI edge; the crossing of Earth's SOI — where, how fast, and WHEN — is the
  output, and the coast starts there, days after the date on the bar. That
  direction is what makes the launch date matter: the same release a fortnight
  later leaves along a different heading and reaches a different part of the
  solar system (`Notes/decisions.md`, 2026-09-01).
