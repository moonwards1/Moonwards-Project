...
import transferLeg, { computeLeg, stateAtElapsed, degAtDay, dayAtDeg, MISS_WARN_AU,
	copyWaypoints, legFor, nearestApproach,
...
// ---- one coast ------------------------------------------------------------
// A waypoint edit moves the emitted state and everything downstream of it in
// the same recompute that redraws the arc.

// A coast state well clear of any body, so these tests measure the waypoint
// burn and not an encounter.
...
test("coast: the emitted packet and dvUsed follow the waypoints as edited", function () {
	var a = coastUpdate({ waypoints: WP_A, legDays: 300, destination: "" });
	var b = coastUpdate({ waypoints: WP_B, legDays: 300, destination: "" });
	assert.ok(O.vMag(O.vSub(b.packet.data.r, a.packet.data.r)) > 1e8,
		"the two waypoint settings should give visibly different flights");
	assert.equal(a.packet.data.dvUsed, 0, "WP_A is a zero burn");
	assert.ok(Math.abs(b.packet.data.dvUsed - 60) < 1e-9, "got " + b.packet.data.dvUsed);
});
...
test("coast: a waypoint edit reaches the arrival leg in the same recompute", function () {
	...
	function arrivalAfterEdit(waypoints) {
		...
		world.set({ stage: coast.id, params: { waypoints: waypoints } });
		...
		return engine.resultFor(arrival.id).output.data.r;
	}
	var a = arrivalAfterEdit(copyWaypoints(WP_A));
	var b = arrivalAfterEdit(copyWaypoints(WP_B));
	assert.ok(O.vMag(O.vSub(b, a)) > 1e6, "the arrival leg did not follow the waypoint edit");
});
