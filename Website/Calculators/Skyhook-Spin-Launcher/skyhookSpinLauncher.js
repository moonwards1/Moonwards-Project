...
let tether = {
	...
	let spinContainer = create("div","container",false,formElement,"min-width:330px");
		create("h4",false,"Spin launcher at launch deck (outbound)",spinContainer);
		create("span","label","Outbound leg: shorten the skyhook and let the spin launcher make up the release speed, plus a plane change. Target sets the heliocentric apoapsis the ship should reach (~2.77 AU = Ceres).",spinContainer);
		create("br",false,false,spinContainer);
		let spinApoInput = create("input",false,false,spinContainer);
			spinApoInput.name="spinApoapsis"; spinApoInput.value=2.77; spinApoInput.type="number"; spinApoInput.step="0.01"; spinApoInput.min="0.01";
		create("span","label","Target heliocentric apoapsis (AU)",spinContainer);
		create("br",false,false,spinContainer);
		let spinPCInput = create("input",false,false,spinContainer);
			spinPCInput.name="spinPlaneChange"; spinPCInput.value=3.5; spinPCInput.type="number"; spinPCInput.step="any"; spinPCInput.min="0";
		create("span","label","Plane-change angle (°)",spinContainer);
		create("br",false,false,spinContainer);
		let spinModeG = create("input",false,false,spinContainer);
			spinModeG.type="radio"; spinModeG.name="spinSizeMode"; spinModeG.value="g"; spinModeG.checked=true;
		create("span","label","Limit tip g → find arm length:",spinContainer);
		let spinGInput = create("input",false,false,spinContainer);
			spinGInput.name="spinGLimit"; spinGInput.value=10; spinGInput.type="number"; spinGInput.step="any"; spinGInput.min="0.01";
		create("span","label","g",spinContainer);
		create("br",false,false,spinContainer);
		let spinModeL = create("input",false,false,spinContainer);
			spinModeL.type="radio"; spinModeL.name="spinSizeMode"; spinModeL.value="L";
		create("span","label","Set arm length → find tip g:",spinContainer);
		let spinLInput = create("input",false,false,spinContainer);
			spinLInput.name="spinArmLen"; spinLInput.value=30; spinLInput.type="number"; spinLInput.step="any"; spinLInput.min="0.01";
		create("span","label","km",spinContainer);
		create("br",false,false,spinContainer);
		let spinTipInput = create("input",false,false,spinContainer);
			spinTipInput.name="spinTipLoad"; spinTipInput.value=4200; spinTipInput.type="number"; spinTipInput.step="any"; spinTipInput.min="0.001";
		create("span","label","Max load at arm tip (ship, t)",spinContainer);
		create("br",false,false,spinContainer);
		let spinHubInput = create("input",false,false,spinContainer);
			spinHubInput.name="spinHubMass"; spinHubInput.value=50000; spinHubInput.type="number"; spinHubInput.step="any"; spinHubInput.min="0.001";
		create("span","label","Counterweight / hub mass (t)",spinContainer);
		create("br",false,false,spinContainer);
		let spinLaunchInput = create("input",false,false,spinContainer);
			spinLaunchInput.name="spinLaunches"; spinLaunchInput.value=1; spinLaunchInput.type="number"; spinLaunchInput.step="any"; spinLaunchInput.min="0.01";
		create("span","label","Launches per day",spinContainer);
		create("br",false,false,spinContainer);
		create("span",false,"Arm material",spinContainer);
		create("br",false,false,spinContainer);
		let spinStrInput = create("input",false,false,spinContainer);
			spinStrInput.name="spinStrength"; spinStrInput.value=42; spinStrInput.type="number"; spinStrInput.step="any"; spinStrInput.min="0.01";
		create("span","label","Tensile strength (GPa)",spinContainer);
		let spinDenInput = create("input",false,false,spinContainer);
			spinDenInput.name="spinDensity"; spinDenInput.value=1400; spinDenInput.type="number"; spinDenInput.step="any"; spinDenInput.min="1";
		create("span","label","Density (kg/m³)",spinContainer);
		let spinSafInput = create("input",false,false,spinContainer);
			spinSafInput.name="spinSafety"; spinSafInput.value=2; spinSafInput.type="number"; spinSafInput.step="0.1"; spinSafInput.min="1";
		create("span","label","Safety factor",spinContainer);
...
let output = create("div","#output",false,mainContainer);
	...
	let spinOut = create("div","container",false,output,"min-width:360px");
		create("h4","resultHead","Spin launcher (outbound to apoapsis)",spinOut);
		create("p","#sl-status",false,spinOut,"color:#a40;font-size:0.85em");
		create("p","#sl-vhelio",false,spinOut);
		create("p","#sl-vinfE",false,spinOut);
		create("p","#sl-vinfM",false,spinOut);
		create("p","#sl-reqLD",false,spinOut);
		create("p","#sl-natLD",false,spinOut);
		create("p","#sl-boost",false,spinOut);
		create("p","#sl-vperp",false,spinOut);
		create("p","#sl-vspin",false,spinOut);
		create("p","#sl-arm",false,spinOut);
		create("p","#sl-tipg",false,spinOut);
		create("p","#sl-rate",false,spinOut);
		create("p","#sl-period",false,spinOut);
		create("p","#sl-tilt",false,spinOut);
		create("p","#sl-taper",false,spinOut);
		create("p","#sl-atip",false,spinOut);
		create("p","#sl-armmass",false,spinOut);
		create("p","#sl-totmass",false,spinOut);
		create("p","#sl-hub",false,spinOut);
		create("p","#sl-epay",false,spinOut);
		create("p","#sl-fly",false,spinOut);
		create("p","#sl-power",false,spinOut);
		create("p","#sl-Lone",false,spinOut);
		create("p","#sl-gyro",false,spinOut);
		create("p","#sl-recoil",false,spinOut);
		create("p","#sl-kick",false,spinOut);
		create("p","#sl-mr",false,spinOut);
		create("p","#sl-timing",false,spinOut);
		create("p","#sl-tumble",false,spinOut);
	create("br",false,false,output);
...
//── Spin launcher at the launch deck (outbound to a heliocentric apoapsis) ──
//   Invert the Moon→Earth→Sun chain: target apoapsis sets the heliocentric speed
//   needed at the parent's distance, hence v∞ at Earth, hence v∞ from the Moon,
//   hence the launch-deck speed required. The skyhook supplies its natural deck
//   speed; the spin launcher adds the rest (prograde) plus the plane change.
	(function(){
		var note = function(id,msg){ var el=document.getElementById(id); if(el){ el.innerText=msg; } };
		var siNum = function(n,d){ if(!isFinite(n)) return "—"; var a=Math.abs(n);
			if(a>=1e18)return (n/1e18).toFixed(d)+" E"; if(a>=1e15)return (n/1e15).toFixed(d)+" P";
			if(a>=1e12)return (n/1e12).toFixed(d)+" T"; if(a>=1e9)return (n/1e9).toFixed(d)+" G";
			if(a>=1e6)return (n/1e6).toFixed(d)+" M"; if(a>=1e3)return (n/1e3).toFixed(d)+" k"; return n.toFixed(d); };
		var fmtEnergy=function(j){ return siNum(j,2)+"J"; };
		var fmtL=function(l){ return siNum(l,2)+"kg·m²/s"; };
		var fmtLen=function(m){ return Math.abs(m)>=1000?(m/1000).toFixed(2)+" km":m.toFixed(1)+" m"; };
		var fmtVel=function(v){ if(!isFinite(v))return "—"; var a=Math.abs(v);
			if(a<1e-3)return (v*1e6).toFixed(2)+" µm/s"; if(a<1)return (v*1e3).toFixed(2)+" mm/s"; return v.toFixed(2)+" m/s"; };
		var g0v = Const.g0;
		var ids=["sl-vhelio","sl-vinfE","sl-vinfM","sl-reqLD","sl-natLD","sl-boost","sl-vperp","sl-vspin","sl-arm","sl-tipg","sl-rate","sl-period","sl-tilt","sl-taper","sl-atip","sl-armmass","sl-totmass","sl-hub","sl-epay","sl-fly","sl-power","sl-Lone","sl-gyro","sl-recoil","sl-kick","sl-mr","sl-timing","sl-tumble"];
		var haveChain = !!(moon.orbit && moon.orbit.system && moon.orbit.system.orbit && moon.orbit.system.orbit.system);
		if(!haveChain){
			note("sl-status","This section needs a body that is a moon of a planet orbiting a star (e.g. the Moon). The selected body lacks that Moon→planet→star chain, so the outbound-to-apoapsis calc is unavailable.");
			ids.forEach(function(id){ note(id,""); });
			return;
		}
		note("sl-status","");
		var GMsun=Number(moon.orbit.system.orbit.system.GM), aEarth=Number(moon.orbit.system.orbit.a),
		    Vearth=Number(moon.orbit.system.orbit.vela), Vmoon=Number(moon.orbit.vela),
		    GMearth=Number(moon.orbit.system.GM), aMoon=Number(moon.orbit.a), GMmoon=Number(moon.GM),
		    rLD=moon.radius+tether.launchDeck;

		var Atarget=gnum("spinApoapsis")*Const.AU;
		var aTrans=(aEarth+Atarget)/2;
		var vHelio=Math.sqrt(GMsun*(2/aEarth - 1/aTrans));   // heliocentric speed needed at parent distance (perihelion of transfer)
		var vinfEarth=vHelio-Vearth;                          // + = outward
		var vRelLunar=Math.sqrt(vinfEarth*vinfEarth + 2*GMearth/aMoon);  // Earth-relative speed at lunar distance
		var vinfMoon=vRelLunar-Vmoon;                         // Moon-relative v∞ (released prograde, Moon's motion helps)
		var reqLD=Math.sqrt(vinfMoon*vinfMoon + 2*GMmoon/rLD);// required inertial speed at the launch deck
		var natLD=tether.launchDeckVel;                       // what the skyhook itself provides
		var boost=reqLD-natLD;                                // spin launcher's in-plane (prograde) share
		var dInc=gnum("spinPlaneChange")*Math.PI/180;
		var vPerp=OrbitalMath.planeChangeComponent(vHelio,dInc);
		var vSpin=OrbitalMath.spinReleaseSpeed(boost,vPerp);
		var tilt=OrbitalMath.spinPlaneTilt(boost,vPerp);

		var mode=(form["spinSizeMode"] && form["spinSizeMode"].value) || "g";
		var armLen, aMax;
		if(mode==="L"){ armLen=gnum("spinArmLen")*1000; aMax=OrbitalMath.spinTipAccel(vSpin,armLen);
			if(form["spinGLimit"]){ form["spinGLimit"].value=(aMax/g0v).toFixed(2); } }
		else { aMax=gnum("spinGLimit")*g0v; armLen=OrbitalMath.spinArmLength(vSpin,aMax);
			if(form["spinArmLen"]){ form["spinArmLen"].value=(armLen/1000).toFixed(2); } }
		var omegaArm=OrbitalMath.spinRate(vSpin,armLen);

		var armSig=gnum("spinStrength")*1e9, armRho=gnum("spinDensity"), armSafe=gnum("spinSafety");
		var sigmaEff=armSig/armSafe;
		var spinTaper=OrbitalMath.spinTaperRatio(vSpin, sigmaEff/armRho);
		var mTip=gnum("spinTipLoad")*1000, Mhub=gnum("spinHubMass")*1000;
		var aTipAcc=omegaArm*omegaArm*armLen, ATip=(mTip*aTipAcc)/sigmaEff;
		var cc=armRho*omegaArm*omegaArm/(2*sigmaEff), steps=2000, dr=armLen/steps, armMass=0, armI=0;
		for(var k=0;k<=steps;k++){ var r=k*dr; var A=ATip*Math.exp(cc*(armLen*armLen-r*r));
			var w=(k===0||k===steps)?0.5:1; armMass+=w*armRho*A*dr; armI+=w*armRho*A*r*r*dr; }
		var hubTension=sigmaEff*ATip*spinTaper;
		var totalStruct=2*armMass+mTip;
		var ePay=0.5*mTip*vSpin*vSpin;
		var eFly=2*0.5*armI*omegaArm*omegaArm + 0.5*mTip*vSpin*vSpin;
		var nDay=gnum("spinLaunches"), avgPower=nDay*ePay/86400;
		var Lone=(armI+mTip*armLen*armLen)*omegaArm;
		var gyro=tether.angularVelocity*Lone;                 // arm spin precesses with the skyhook's own rotation
		var recoil=mTip*vSpin, kick=recoil/Mhub, mr=mTip/Mhub;
		var vErrMs=vSpin*omegaArm*1e-3, tumbleDeg=omegaArm*180/Math.PI;

		note("sl-vhelio","Heliocentric departure speed (velocity being tilted): "+myRound(vHelio,1)+" m/s");
		note("sl-vinfE","v∞ needed at Earth: "+myRound(vinfEarth,1)+" m/s");
		note("sl-vinfM","v∞ needed from the Moon (at launch deck): "+myRound(vinfMoon,1)+" m/s");
		note("sl-reqLD","Required launch-deck speed: "+myRound(reqLD,1)+" m/s");
		note("sl-natLD","Skyhook provides at launch deck: "+myRound(natLD,1)+" m/s");
		note("sl-boost","Spin in-plane (prograde) boost: "+myRound(boost,1)+" m/s"+(boost<0?"  (skyhook overshoots target — small retro/prograde trim)":""));
		note("sl-vperp","Out-of-plane component: "+myRound(vPerp,1)+" m/s");
		note("sl-vspin","Required release (tip) speed: "+myRound(vSpin,1)+" m/s");
		note("sl-arm","Arm length: "+fmtLen(armLen));
		note("sl-tipg","Tip acceleration: "+myRound(aMax/g0v,2)+" g ("+myRound(aMax,1)+" m/s²)");
		note("sl-rate","Spin rate: "+omegaArm.toExponential(3)+" rad/s ("+myRound(omegaArm/(2*Math.PI)*60,3)+" rpm)");
		note("sl-period","Rotation period: "+(2*Math.PI/omegaArm>=60?myRound(2*Math.PI/omegaArm/60,2)+" min":myRound(2*Math.PI/omegaArm,1)+" s"));
		note("sl-tilt","Required spin-plane tilt: "+myRound(tilt*180/Math.PI,2)+"°");
		note("sl-taper","Arm taper ratio (hub/tip): "+myRound(spinTaper,4));
		note("sl-atip","Tip cross-section (per arm): "+myRound(ATip,4)+" m² (Ø "+myRound(2*Math.sqrt(ATip/Math.PI),2)+" m)");
		note("sl-armmass","Mass of one arm: "+fmtMass(armMass));
		note("sl-totmass","Total structure (2 arms + counter-mass): "+fmtMass(totalStruct));
		note("sl-hub","Peak hub tension (per arm): "+fmtForce(hubTension));
		note("sl-epay","Energy to payload per launch: "+fmtEnergy(ePay));
		note("sl-fly","Stored spin energy (arms + counter-mass): "+fmtEnergy(eFly));
		note("sl-power","Avg spin-up power (payload, "+myRound(nDay,2)+"/day): "+fmtPower(avgPower));
		note("sl-Lone","Angular momentum per arm: "+fmtL(Lone)+"  (net ≈ 0, counter-rotating)");
		note("sl-gyro","Gyro torque per arm (orbital precession): "+fmtTorque(gyro)+" (cancels at hub)");
		note("sl-recoil","Release recoil impulse: "+siNum(recoil,2)+"N·s");
		note("sl-kick","Counterweight velocity kick: "+fmtVel(kick));
		note("sl-mr","Tip-load / hub-mass ratio: "+(mr*100>=0.01?myRound(mr*100,2):(mr*100).toExponential(2))+" %");
		note("sl-timing","Velocity error per ms of slip: "+myRound(vErrMs,2)+" m/s");
		note("sl-tumble","Tumble handed to payload: "+myRound(tumbleDeg,3)+" °/s");
	})();
