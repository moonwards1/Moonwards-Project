...
let formElement = create("form","#input",false,mainContainer);
	let formContainer = create("div","container",false,formElement);
		...
		create("br",false,false,formContainer);
		let counterweightInput = create("input",false,false,formContainer);
			counterweightInput.name = "counterweight";
			counterweightInput.value = 2000;
		create("span","label","Tether counterweight altitude (km)",formContainer);
		create("br",false,false,formContainer);
		create("br",false,false,formContainer);
...
let tether = {
	materials: {
		"silica": {
			displayName: "Lunar Silica (13 GPa, 2.3 g/cm³)",
			strength: 13000000000,
			density: 2300
		},
		"bestsilica": {
			displayName: "Best lunar silica (17 GPa, 2.3 g/cm³)",
			strength: 17000000000,
			density: 2300
		},
		"earlylunarglass": {
			displayName: "Early lunar glass (3 GPa, 2.7 g/cm³)",
			strength: 3000000000,
			density: 2700
		},
		"ecnt": {
			displayName: "Early Carbon Nanotube (18 GPa, 1.45 g/cm³)",
			strength: 18000000000,
			density: 1450
		},
		"mwcnt": {
			displayName: "Mature Carbon Nanotube (42 GPa, 1.4 g/cm³)",
			strength: 42000000000,
			density: 1400
		},
		"zylon": {
...
let output = create("div","#output",false,mainContainer);
	...
	create("br",false,false,output);
	let geometryContainer = create("div","container",false,output);
		create("h4",false,"Upper tether geometry",geometryContainer);
		create("p","#upperCoM",false,geometryContainer);
		create("p","#counterweightMass",false,geometryContainer);
	create("br",false,false,output);
...
//calculation of taper ratio
	...
	var highMomentSum = 0;
	var highInverseSum = 0;
	var iteratorLimit = 1000;
	var lowCrosses = [];
	var highCrosses = [];
//numerical way to find the tether mass. Also used for plotting the width of it
	for(var i=0;i<iteratorLimit;i++){
		...
		highInverseSum += 1/highCross;
		highMomentSum += (tether.centre + i*(tether.top - tether.centre)/iteratorLimit) / highCross;
		if(i % 10 == 0){
	...
	// CoM of upper tether (altitude above surface, in metres).
	// A(r) = A(r_centre)/highCross(r), so CoM = Σ(alt/highCross) / Σ(1/highCross)
	tether.highCoM = highMomentSum / highInverseSum;

	var cwAlt = gnum("counterweight") * 1000;
	// Moment balance: M_up*(CoM - centre) + M_cw*(cwAlt - centre) = 0
	// => M_cw = M_up * (CoM - centre) / (centre - cwAlt)
	if(cwAlt < tether.centre && cwAlt >= 0){
		tether.counterweightMass = tether.highMass * (tether.highCoM - tether.centre) / (tether.centre - cwAlt);
	}
...
*/
	...
	out("upperCoM","Upper tether center of mass: " + myRound((tether.highCoM - tether.centre)/1000,3) + " km above orbital altitude");
	if(cwAlt < 0){
		warnings += "Counterweight is underground!<br>";
		out("counterweightMass","");
	} else if(cwAlt >= tether.centre){
		warnings += "Counterweight altitude must be below orbital altitude (" + myRound(tether.centre/1000,1) + " km).<br>";
		out("counterweightMass","");
	} else {
		out("counterweightMass","Required counterweight: " + myRound(tether.counterweightMass,4) + " x payload mass at " + myRound(cwAlt/1000,2) + " km altitude");
	}
